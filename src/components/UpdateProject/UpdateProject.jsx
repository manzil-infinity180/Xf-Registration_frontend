import { getPost, queryclient, updateProjectPost } from "../../utils/http";
    ...
    const {mutate}=useMutation({
        ...
        onMutate : async (data)=>{
           // getting the data 
            ...
            await queryclient.cancelQueries({queryKey: ['getme',id]});
            // previous stored queries
            const previousEvent= queryclient.getQueriesData(['getme',id]);
             // setting the data 
            queryclient.setQueriesData(['getme',id],newData);
            return {previousEvent};
        },
        onError: (error,data,context)=>{
            toast.error(error);
            queryclient.setQueriesData(['getme',id],context.previousEvent);
          },
          onSettled: ()=>{
            // invalidating the query that we update the outdated data 
            toast.success("Updated Successfully");
            queryclient.invalidateQueries(['getme',id]);
          }
    ...
    <main>
        <section className='section_container_login section_signin'>
          ...
          <div className='signin_content'>
            <p>Update your Projects</p>
            <p className='signin_description'>
               From this page,you can update your projects that will directly reflect the project section of your profile
            </p>
          </div>
        <div className='main_login_box'>
                <form className='form_login' onSubmit={handleSubmit}>
                    ...
                    { data && <>
                        <InputFieldUpdate name={"title"} type={"text"} placeholder={"Title of Project"}
                      value={data.title}>Title</InputFieldUpdate>
                    <InputFieldUpdate name={"description"} type={"text"} placeholder={"Description of Project"}
                    value={data.description}>Description</InputFieldUpdate>
                    <InputFieldUpdate name={"techstack"} type={"text"} placeholder={"Tech Stack like nodejs,reactjs,mongodb"}
                    value={data.techstack}>Tech Stack</InputFieldUpdate>
                    <InputFieldUpdate name={"repolink"} type={"text"} placeholder={"Repository Link of Project"}
